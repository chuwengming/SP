...
class MqttManager extends EventEmitter {
    ...
    connect(config, source = 'unknown', plugId = 'defaultPlug') {
        ...
        let protocol = 'mqtt';
        if (String(config.port) === '8883') protocol = 'mqtts';
        else if (String(config.port) === '8084') protocol = 'wss';
        else if (String(config.port) === '8083') protocol = 'ws';

        const path = (protocol === 'ws' || protocol === 'wss') ? '/mqtt' : '';
        const connectUrl = `${protocol}://${config.broker}:${config.port}${path}`;
        ...
        client.on('error', (err) => {
            ...
            this.emit('mqtt_error', clientId, err);
        });
